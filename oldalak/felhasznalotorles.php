...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['fiokTorles']) && isset($_POST['TorlesEmail']) && isset($_POST['TorlesJelszo'])) {
    ...
    if ($user && $db->verifyPassword($jelszo, $user['jelszo'])) {
        ...
        if ($stmt_delete_user->execute()) {
            ...
            echo '<script>alert("Hiba történt a fiók törlése közben.");</script>';
        }
    } else {
...
<!--KARTYA-->
    <div class="card position-relative">
        ...
        <div class="card-header text-center text-md-center">
            <h1 class="mb-0 fs-5">Felhasználó törlése</h1>
        </div>
        
        <!--KARTYA_TESTE-->
        <div class="card-body py-3">
            <form id="deleteForm" action="#" method="post" novalidate>
                <div class="from-group mb-4">

                    <!--E-MAIL-->
                    <label for="TorlesEmail"><b>E-mail cím:</b></label>
                    <div class="input-group">
                        ...
                        <input type="email" class="form-control" placeholder="Név@példa.com" id="TorlesEmail" name="TorlesEmail" autofocus required>
                    </div>

                    <hr>
                    
                    <!--JELSZO-->
                    <label for="TorlesJelszo"><b>Jelszó:</b></label>
                    <div class="input-group">
                        ...
                        <input type="password" class="form-control" placeholder="Jelszó" id="TorlesJelszo" name="TorlesJelszo" autofocus required>
                    </div>
                    <hr>
                </div>
                ...
                <!--BEJELENTKEZES_GOMB-->
                <div class="d-grid">
                    <button type="submit" name="fiokTorles" class="btn btn-danger" value="true" onclick="return confirm('Biztosan törölni szeretnéd a fiókodat?')">Fiók törlés</button>
                </div>
            </form>
        </div>
    </div>